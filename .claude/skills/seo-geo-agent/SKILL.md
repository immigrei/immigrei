...
- **Type A drafts publish themselves — no per-page code.** `lib/contentPages.ts`
  renders every draft in `content/marketing/drafts/` that clears all publish
  gates (`status: approved|published`, `reviewed_by` set, `compliance_check`
  starting with `PASS`, ISO `verificado_em`, no open `<!-- VERIFY` markers,
  valid YAML frontmatter). `type: status` → `/status/<slug>`;
  `type: decoder | form-lifecycle` → `/entenda/<slug>`; `faq-bank` never
  becomes a page. Route, metadata, JSON-LD, OG image, sitemap entry and
  llms.txt line all come from that one loader — your promote job is the gate
  check + GEO audit, not new route files. `getPendingDrafts()` lists what is
  blocked and why.
...
- One site-wide `Organization` + `WebSite` node — **already in
  `app/layout.tsx`** (`@id` `https://immigrei.app/#organization`). Per-page
  `author`/`publisher` reference that `@id`; never redeclare it per page.
...
- IndexNow: the key lives in `lib/indexnow.ts` (file served from
  `public/<key>.txt`, allow-listed in `proxy.ts`). After a production deploy,
  run `npx tsx scripts/indexnow.ts` (whole live sitemap) or pass specific URLs.
- `llms.txt` / `llms-full.txt` are generated by `lib/llmsTxt.ts` from the same
  catalogs as the sitemap — a new catalog-driven route family belongs there too.
- Check `app/robots.ts` doesn't block the new route, and that the route is in
  `proxy.ts`'s `isPublicRoute` (a `.txt` path needs an explicit entry — the
  middleware matcher doesn't skip it).
