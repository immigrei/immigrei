...
//
// `webhookUrl` lets a caller target a different channel (its own Incoming
// Webhook, e.g. SLACK_STRIPE_WEBHOOK_URL for #stripe-alertas) instead of the
// default alerts channel — each Slack Incoming Webhook is bound to one fixed
// channel at creation, so routing to a second channel needs a second URL.
export async function notifySlackAlert(text: string, webhookUrl?: string) {
  const url = webhookUrl || process.env.SLACK_ALERTS_WEBHOOK_URL;
  if (!url) {
